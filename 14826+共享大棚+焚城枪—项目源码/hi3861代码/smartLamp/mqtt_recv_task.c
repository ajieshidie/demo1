...
uint8_t cJSON_Parse_Payload(uint8_t *payload)
{
    ...
    (sys_msg_data.pump_D1 == 1) ? set_D1(true) : set_D1(false);
    (sys_msg_data.lamp_D2 == 1) ? set_D2(true) : set_D2(false);
    (sys_msg_data.FAN_D3 == 1) ? set_D3(true) : set_D3(false);
    (sys_msg_data.Elc_D4 == 1) ? set_D4(true) : set_D4(false);
    (sys_msg_data.auto_mode == 1) ? (auto_bool = 1) : (auto_bool = 0);
